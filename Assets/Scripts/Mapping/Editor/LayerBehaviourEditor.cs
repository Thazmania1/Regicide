    ...
    public override void OnInspectorGUI()
    {
        ...
        if(GUILayout.Button("Reset grid"))
        {
            foreach(var gameObject in targets)
            {
                ...
                for(int i = 0; i < grid.arraySize; i++) grid.GetArrayElementAtIndex(i).boolValue = false;
        ...
        {
            // If the layer's chunk is a match board, ithecks if there's any active block in the layer (the knight ignores this)
            if(layerBehaviour.GetComponentInParent<ChunkBehaviour>().IsMatchBoard)
            {
    ...
    private void ResetPhysicalGrid(LayerBehaviour layerBehaviour)
    {
        ...
        for(int row = 0; row < GRID_SIZE; row++)
        {
            ...
            for(int col = 0; col < GRID_SIZE; col++)
            {
                ...
                newBlock.transform.parent = layerTransform;
